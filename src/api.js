  ...
  db.query(query, [email], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      res.json({
        ...
        token,
        utez_community: user.utez_community,
        user: {
          name: user.nombre,
          username: user.email,
        },
...
app.post("/create-course", (req, res) => {
  try {
    ...
    const token = req.headers.authorization?.split(" ")[1]; // Maneja el caso en que no haya token
    if (!token) {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        return res.status(200).json({ message: "Curso creado con éxito" });
      }
    );
  } catch (error) {
...
app.delete("/instructors/:id", async (req, res) => {
  ...
  try {
    await db
      .promise()
      .query('UPDATE usuarios SET status = "Desactivado" WHERE id = ?', [id]);
...
app.get("/student-count", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .send("Error al consultar el número de estudiantes");
    }
    res.json(results[0]);
  });
});
...
app.get("/course-count", verifyToken, async (req, res) => {
  ...
  const query =
    "SELECT COUNT(*) AS courseCount FROM cursos WHERE instructor_id = ?";
...
app.get("/best-selling-courses", verifyToken, async (req, res) => {
  ...
  const query =
    "SELECT title, ventas AS sales, precio AS amount, image FROM cursos WHERE instructor_id = ? ORDER BY ventas DESC LIMIT 5";
  db.query(query, [instructorId], (err, results) => {
    if (err) {
      console.error(err);
...
app.put("/user/updateemail", verifyToken, async (req, res) => {
  ...
  try {
    const [result] = await db
      .promise()
      .query("UPDATE usuarios SET email = ? WHERE id = ?", [email, userId]);
...
app.put("/user/updatepassword", verifyToken, async (req, res) => {
  ...
  if (!currentPassword || !newPassword) {
    return res
      .status(400)
      .send("La contraseña actual y la nueva contraseña son requeridas.");
  }
...
app.post("/forgot-password", (req, res) => {
  ...
  const query =
    "UPDATE usuarios SET resetPasswordToken = ?, resetPasswordExpires = ? WHERE email = ?";
  db.query(query, [token, formattedExpirationDate, email], (err, result) => {
    ...
    const msg = {
      ...
      text:
        `Estás recibiendo este correo porque tú (o alguien más) ha solicitado el restablecimiento de la contraseña de tu cuenta.\n\n` +
...
app.post("/reset-password", (req, res) => {
  ...
  const query =
    "SELECT * FROM usuarios WHERE resetPasswordToken = ? AND resetPasswordExpires > UTC_TIMESTAMP()";
...
app.put("/update-course/:id", (req, res) => {
  const { id } = req.params;
  const { title, description, precio, level, category, status, instructor_id } =
    req.body;
  ...
  if (
    ...
    return res
      .status(400)
      .send("Faltan datos necesarios para la actualización.");
  }
  ...
  db.query(
    query,
    [title, description, precio, level, category, status, instructor_id, id],
    (err, result) => {
      if (err) {
...
app.post("/enroll", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const [curso] = await connection.query(
      'SELECT * FROM cursos WHERE id = ? AND status = "aprobado"',
      [cursoId]
    );
    if (curso.length === 0) {
      ...
      return res
        .status(404)
        .send("Curso no encontrado o no disponible para inscripción.");
    }

    // Verificar que el usuario es un estudiante
    const [usuario] = await connection.query(
      'SELECT * FROM usuarios WHERE id = ? AND utez_community = "estudiante"',
      [userId]
    );
    if (usuario.length === 0) {
      await connection.rollback();
      return res
        .status(403)
        .send("Solo los estudiantes pueden inscribirse en cursos.");
    }
    ...
    const [updateVentas] = await connection.query(
      "UPDATE cursos SET ventas = ? WHERE id = ?",
      [nuevoTotalVentas, cursoId]
    );
...
app.get("/course-enrollments/:cursoId", verifyToken, async (req, res) => {
  ...
  const [course] = await db
    .promise()
    .query("SELECT id FROM cursos WHERE id = ? AND instructor_id = ?", [
      cursoId,
      userId,
    ]);
  if (course.length === 0) {
